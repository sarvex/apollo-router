...
If you issue JWTs via a popular third-party IdP (Auth0, Okta, PingOne, etc.), adding JWT authentication to your router is comparatively straightforward. If you use your own custom IdP, additional configuration is required.
...
4. Your router validates the extracted JWT using a corresponding [JSON Web Key](https://www.rfc-editor.org/rfc/rfc7517) (**JWK**).
    - Your router obtains all of its known JWKs from URLs that you specify in its configuration file. Each URL provides one or more keys as a single JSON object called a [JWK Set](https://www.rfc-editor.org/rfc/rfc7517#section-5) (or a **JWKS**).
...
6. Your customizations can handle the request differently depending on the details of the extracted claims, and/or you can propagate the claims to subgraphs for more granular access control.
...
Make sure to provide your config file to the router on startup!
...
**Required.** A list of URLs for the JWK Sets (JWKS) that the router should use. Each list item can be an `https://` or a `file://` URL.

- **If you use a third-party IdP,** consult its documentation to determine its JWKS URL.
- **If you use your own custom IdP,** you need to make its JWKS available at a router-accessible URL if you haven't already. For more information, see [Creating a JWKS](#creating-a-jwks).
...
Whenever the router receives a JWT with a key ID (`kid`) that it doesn't recognize, it fetches up-to-date JWK Sets from its [`jwks_urls`](#jwks_urls), _if_ it's been _at least_ the amount of time specified by `cooldown` since it last fetched.
...
Because claims are added to the context, you can define custom logic for handling each request based on the details of its claims. You can define this logic as a Rhai script or a native Rust plugin (for more on these, see [Customizations for the Apollo Router](../customizations/overview/)).
...
Because the Apollo Router handles validating incoming JWTs, you rarely need to pass those JWTs to individual subgraphs. Instead, you usually want to [pass JWT _claims_ to subgraphs](#example-forwarding-claims-to-subgraphs).
...
>
> Consult this section if you use a custom IdP that doesn't currently publish a JWKS.
